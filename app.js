...
function App() {
  ...
  return (
    <div style={{fontFamily:"'DM Sans',sans-serif",maxWidth:680,margin:"0 auto",position:"relative",paddingBottom:70,color:"#1a1a1a",background:"#fff",minHeight:"100vh",padding:"0 16px"}}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&family=DM+Serif+Display&display=swap" rel="stylesheet"/>

      {notif && <div style={{position:"fixed",top:20,left:"50%",transform:"translateX(-50%)",background:"#1a1a1a",color:"#fff",padding:"10px 20px",borderRadius:50,fontSize:13,fontWeight:600,zIndex:100,animation:"slideDown .3s ease",boxShadow:"0 8px 32px rgba(0,0,0,.2)"}}>{notif}</div>}

      <style>{`
        @keyframes slideDown{from{opacity:0;transform:translateX(-50%) translateY(-20px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}
        @keyframes fadeIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
        @keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.05)}}
        .cd{background:#fff;border:1px solid #e8e5e0;border-radius:14px;padding:14px;margin-bottom:10px;animation:fadeIn .4s ease}
        .cd:hover{border-color:#d0ccc5}
        .bt{border:none;cursor:pointer;font-family:'DM Sans',sans-serif;font-weight:600;border-radius:10px;transition:all .15s}
        .bt:active{transform:scale(.97)}
        .bp{background:#1a1a1a;color:#fff;padding:12px 24px;font-size:14px}.bp:hover{background:#333}
        .bo{background:transparent;border:1.5px solid #e8e5e0;color:#1a1a1a;padding:8px 16px;font-size:13px}.bo:hover{background:#f8f7f5}
        .nv{background:transparent;border:none;cursor:pointer;padding:6px 12px;font-size:13px;font-weight:600;color:#999;border-radius:8px;font-family:'DM Sans',sans-serif;transition:all .15s}
        ...
        <div>
          {editingTitle ? 
            <input 
              ...
              autoFocus
              style={{fontFamily:"'DM Serif Display',serif",fontSize:24,margin:0,letterSpacing:-.5,border:"none",outline:"2px solid #1a1a1a",borderRadius:8,padding:"4px 8px",background:"#fff"}}
            />
          :
            <h1 
              onClick={()=>setEditingTitle(true)} 
              style={{fontFamily:"'DM Serif Display',serif",fontSize:24,margin:0,letterSpacing:-.5,cursor:"pointer"}}
            >
              {appTitle}
            </h1>
          }
        <p style={{color:"#999",fontSize:12,margin:"4px 0 0",fontWeight:500}}>{DAYS[dow]} {today.getDate()} {MONTHS[today.getMonth()]}</p></div>
        <span style={{fontSize:12,fontWeight:700,background:"#f0eeea",padding:"5px 12px",borderRadius:20}}>🔥 {stats.total}</span>
      </div>

      {/* Nav */}
      <div style={{display:"flex",gap:4,padding:"10px 0 14px",borderBottom:"1px solid #f0eeea",marginBottom:14,flexWrap:"wrap"}}>
